...
async function main(): Promise<void> {
  ...
  for (const product of seedData.products) {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await Promise.all([
        ...
        tx.productIngredient.deleteMany({
          where: { productId: savedProduct.id },
        }),
        tx.productGalleryImage.deleteMany({
          where: { productId: savedProduct.id },
        }),
      ]);
      await tx.productBenefit.createMany({
        data: benefits.map((text, position) => ({
      ...
      await tx.productGalleryImage.createMany({
        data: galleryImages.map((url, position) => ({
          productId: savedProduct.id,
          url,
          altText: `${product.name} image ${position + 1}`,
          position,
        })),
      });
    });
  ...
  console.log(
    `Seed completed: ${seedData.collections.length} collections, ${seedData.products.length} products`
  );
}
