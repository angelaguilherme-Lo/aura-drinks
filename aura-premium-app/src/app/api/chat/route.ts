...
function answerFromCatalog(message: string) {
  ...
  if (q.includes('checkout') || q.includes('pay') || q.includes('cart')) {
    return {
      message: `${catalog.demoPolicies.checkout} You can review selected drinks, continue to the checkout screen, and complete a simulated purchase flow.`,
    };
  }
  ...
  if (
    ...
    return {
      message: `${catalog.demoPolicies.signIn} ${catalog.demoPolicies.account}`,
    };
  }

  if (q.includes('favorite') || q.includes('favourite') || q.includes('save')) {
    return {
      message: `${catalog.demoPolicies.favorites} Saved items can be viewed later on the favorites page inside the demo account area.`,
    };
  }
  ...
  if (q.includes('floral')) {
    ...
    return {
      message: `For a floral taste, I recommend ${formatProductList(matches)}. These drinks lean into soft botanical and blossom-led flavor profiles.`,
    };
  }
  ...
  if (q.includes('summer')) {
    ...
    return {
      message: `For summer, I recommend ${formatProductList(matches)}. These flavors are brighter, fruit-forward, and especially refreshing.`,
    };
  }
  ...
  return {
    message: `Hello — I’m the Aura assistant. I can help with flavors, collections, ingredients, favorites, accounts, and checkout questions. A few drinks to explore first are ${formatProductList(featured)}.`,
  };
}
